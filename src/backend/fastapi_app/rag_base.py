...
class RAGChatBase(ABC):
    ...
    def get_params(self, messages: list[ChatCompletionMessageParam], overrides: ChatRequestOverrides) -> ChatParams:
        ...
        enable_text_search = overrides.retrieval_mode in ["texto", "hibrido", None]
        enable_vector_search = overrides.retrieval_mode in ["vetor", "hibrido", None]

        original_user_query = messages[-1]["content"]
        if not isinstance(original_user_query, str):
            raise ValueError("Conteudo da mensagem mais recente precisa ser string.")
