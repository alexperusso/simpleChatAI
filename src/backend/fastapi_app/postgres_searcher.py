...
class PostgresSearcher:
    def __init__(
        ...
        embed_deployment: Optional[str],  # Nao usado para retrieval_mode="texto"
        embed_model: str,
    ...
    async def search(
        ...
        fulltext_query = f"""
            SELECT id, RANK () OVER (ORDER BY ts_rank_cd(to_tsvector('portuguese', texto), query) DESC)
                FROM {table_name}, plainto_tsquery('portuguese', :query) query
                WHERE to_tsvector('portuguese', texto) @@ query {filter_clause_and}
                ORDER BY ts_rank_cd(to_tsvector('portuguese', texto), query) DESC
                LIMIT 20
            """
        ...
        if query_text is not None and len(query_vector) > 0:
            ...
            raise ValueError("Query de vetor e texto vazias")
        ...
        # Convert resultados em modelos SQLAlchemy
        row_models = []
        for id, _ in results[:top]:
            item = await self.db_session.execute(select(Oraculo).where(Oraculo.id == id))
    ...
    async def search_and_embed(
        ...
        """
        Procura por texto e converte a query em vetor.
        """
        vector: list[float] = []
