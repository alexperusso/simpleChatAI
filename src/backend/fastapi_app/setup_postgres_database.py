...
async def create_db_schema(engine):
    async with engine.begin() as conn:
        logger.info("Habilitando extensao pgvector no Postgres...")
        await conn.execute(text("CREATE EXTENSION IF NOT EXISTS vector"))
        logger.info("Criando tabelas e indices...")
...
async def main():
    parser = argparse.ArgumentParser(description="Criando o schema do Banco de Dados")
    ...
    logger.info("Instalado a extensao do Database e criado as tabelas.")
